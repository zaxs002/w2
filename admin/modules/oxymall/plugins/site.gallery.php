	...
	*/
	...
	function GetCategories() {
		$cats = $this->db->QFetchRowArray(
			"SELECT * FROM {$this->tables['plugin:gallery_cats']} WHERE module_id={$this->tpl_module[mod_id]} ORDER BY cat_order"
		);
	...
	function GetRecords($cat) {
		...
		$items = $this->db->QFetchRowArray(
			"SELECT * " . 
			"FROM {$this->tables['plugin:gallery_items']} " . 
			"WHERE module_id={$this->tpl_module[mod_id]} AND item_cat={$cat[cat_id]} " .
		...
		if (is_Array($items)) {
			foreach ($items as $key => $val) {
				switch ($val["item_type"]) {
					case "1":
						$items[$key]["link"] = "upload/galleries/{$val[item_id]}.jpg";
					break;

					case "2":
						$items[$key]["link"] = "upload/galleries/{$val[item_id]}.mp4";
					break;
