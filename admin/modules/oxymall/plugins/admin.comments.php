...
class COXYMallComments extends CPlugin{
	...
	function DoEvents(){
		...
		if (strstr($_GET["sub"] , "oxymall.plugin.comments.")) {
			$sub = str_replace("oxymall.plugin.comments." , "" ,$_GET["sub"]);
			$action = $_GET["action"];
			...
			switch ($sub) {
				case "landing":
					$data = new CSQLAdmin("comments/" . $sub, $this->__parent_templates,$this->db,$this->tables,$extra);
	...
	*/
	function ExportSubscribers() {
		...
		$subscribers = $this->db->QFetchRowArray("SELECT * FROM {$this->tables['plugin:comments_subscribers']}");
	...
	function StoreNewsletter($record) {
		...
		if ($record["item_status"] == 2) {
			...
			$users = $this->db->QFetchRowArray(
				"SELECT * FROM {$this->tables['plugin:comments_subscribers']} "
			);
			...
			if (is_array($users)) {
				foreach ($users as $key => $user) {
					...
					//save in history
					$this->db->QueryInsert(
						$this->tables['plugin:comments_history'],
						array(
			...
			$this->db->QueryUpdate(
				$this->tables['plugin:comments_items'],
				array(
					"item_status"	=> "3"
				),
